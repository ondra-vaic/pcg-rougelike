...
class OverlappingModel(val inputBitmap: BufferedImage, val N: Int, width: Int, height: Int, periodicInput: Boolean,
    ...
    init {
        ...
        val smx = inputBitmap.width
        val smy = inputBitmap.height
        val sample = Array(smx) { ByteArray(smy) }
        colors = ArrayList()

        for (y in 0 until smy) {
            for (x in 0 until smx) {
                val color = Color(inputBitmap.getRGB(x, y))
                var i = 0
                run breakOut@{
                    colors?.forEach {
                        if (it == color) {
